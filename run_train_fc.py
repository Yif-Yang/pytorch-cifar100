...
parser.add_argument('-exp_name', type=str, default='diversity_mean_loss_fc_step', help='exp_name ')
...
parser.add_argument('-aux_dis_lambda', type=float, default=10, help='aux_dis_lambda loss rate')
parser.add_argument('-main_dis_lambda', type=float, default=0, help='main_dis_lambda loss rate')
...
parser.add_argument('-loss_aux_single', action='store_true', default=True, help='loss_aux_ensemble')
parser.add_argument('-seed', type=int, default=-1, metavar='S', help='random seed (default: 1)')

args = parser.parse_args()
if not os.path.exists('./work_dir'):
      os.mkdir('./work_dir')
exp_name = f"{args.exp_name}_{args.net}_lr{args.lr}_{'close-fc_' if args.close_fc_grad else ''}{'ens_' if args.loss_aux_ensemble else ''}{'sing_' if args.loss_aux_single else ''}dis-length-{args.aux_dis_lambda}{f'_seed_{args.seed}' if args.seed > -1 else ''}_run-{args.runs}"
cmd = f"CUDA_VISIBLE_DEVICES={args.gpu} " \
      "nohup python train_fc_step.py " \
      f"-net {args.net} " \
      f"-lr {args.lr} " \
